...
import (
	...
	jsonv2compactor "pkt.systems/jpact/jsonv2compactor"
)
...
func benchmarkCompact(b *testing.B, name string, data []byte) {
	...
	b.Run(name+"/compactor", func(b *testing.B) {
		b.ReportAllocs()
		var buf bytes.Buffer
		buf.Grow(len(data))
		...
		for b.Loop() {
			buf.Reset()
			reader.Reset(data)
			if err := CompactWriter(&buf, reader, int64(len(data))); err != nil {
				b.Fatalf("CompactWriter failed: %v", err)
			}
		}
	})
	b.Run(name+"/jsonv2compactor", func(b *testing.B) {
		b.ReportAllocs()
		var buf bytes.Buffer
		buf.Grow(len(data))
		reader := bytes.NewReader(data)
		for b.Loop() {
			buf.Reset()
			reader.Reset(data)
			if err := jsonv2compactor.CompactWriter(&buf, reader, int64(len(data))); err != nil {
