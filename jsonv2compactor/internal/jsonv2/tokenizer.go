...
const defaultTokenizerBufCap = 4096
...
func NewTokenizer(r io.Reader, maxBytes int64) *Tokenizer {
	return &Tokenizer{
		rd:  r,
		max: maxBytes,
		buf: make([]byte, 0, defaultTokenizerBufCap),
	}
}

// Reset reuses the tokenizer with a new reader and maxBytes. maxBufCap controls
// how large the internal buffer may grow before it is reallocated.
func (t *Tokenizer) Reset(r io.Reader, maxBytes int64, maxBufCap int) {
	t.rd = r
	t.max = maxBytes
	t.offset = 0
	t.pos = 0
	...
	if maxBufCap <= 0 {
		maxBufCap = defaultTokenizerBufCap
	}
	if cap(t.buf) == 0 || cap(t.buf) > maxBufCap {
		t.buf = make([]byte, 0, defaultTokenizerBufCap)
		return
	}
	t.buf = t.buf[:0]
}
...
func (t *Tokenizer) readString() (Kind, []byte, error) {
	...
	for {
		slice := t.buf[start:]
		n, err := consumeStringResumable(&flags, slice, resume, false)
		if err == nil {
			end := start + n
			t.pos = end
